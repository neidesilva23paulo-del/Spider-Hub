...
do
	...
	-- 2. ESP de Computadores Dinâmico (Corrigido sem travamento de porcentagem)
				...
				for _, v in ipairs(mapValue:GetChildren()) do
					if v.Name == "ComputerTable" and v:IsA("Model") then
						...
						if billboard then
							...
							-- Se o PC foi concluído (tela verde), força 100% imediatamente
							if screen and screen.BrickColor == BrickColor.new("Dark green") then
								pcProgressTracker[v] = 100
							else
								local progress = nil

								-- Verifica TODOS os jogadores próximos digitando (INCLUINDO o LocalPlayer)
								for _, p in ipairs(Players:GetPlayers()) do
									if p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p:FindFirstChild("TempPlayerStatsModule") then
										local anim = p.TempPlayerStatsModule:FindFirstChild("CurrentAnimation")
										if anim and anim.Value == "Typing" and v:FindFirstChild("Screen") then
											local dist = (p.Character.HumanoidRootPart.Position - v.Screen.Position).Magnitude
											if dist < 15 then
												local progressVal = p.TempPlayerStatsModule:FindFirstChild("ActionProgress")
												if progressVal then
								...
								-- Se houver alguém digitando, atualiza. Se não, mantém o valor ou inicia em 0%
								if progress then
